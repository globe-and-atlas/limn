...
- **Screen** (`focused-triage`, default active) — a curated investigation workflow with a **Spill Evidence Timeline**, primary evidence-lens row, collapsed **Gate Diagnostics** drawer (`ndsi`, `si`, `hcai`, `ndoi`, `csi`, `hmri`), and a separate collapsed negative-result drawer for PWCI, ASAI, and OBEC. The timeline derives Reference (−180 d), Before (−30 d), Event, After (+30 d), Late (+180 d), and Latest targets from the bookmark event date, clamps them to viewer coverage, and preserves the selected temporal view while switching among RGB/MNDWI/LBI/PWCI/ASAI. Reference↔Event and Event↔Late shortcuts use COG-compatible swipe comparison. The OSM toggle is mapped-road context, not a road/pad exclusion mask. Diagnostic labels describe band responses and explicitly avoid chemical-retrieval claims.
