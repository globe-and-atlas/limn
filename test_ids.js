const ids = ['sentinel-1-grd', 'SENTINEL1_GRD'];

async function test(id) {
    const evalscript = `//VERSION=3
function setup() { return { input: [{ datasource: "${id}", bands: ["VV"] }], output: { bands: 1 } }; }
function evaluatePixel(samples) { return [1]; }`;
    ...
    console.log(`${id}: ${text.substring(0, 300)}`);
}

async function runAll() {
    for (let id of ids) await test(id);
}
runAll();
