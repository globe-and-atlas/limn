...
**Resolved (2026-07-20): path (a) executed and then narrowed again.** PUBLIC_SCIENCE_GUIDE.md was retitled and repositioned as a methodology / negative-result paper (v2.0): new title, rewritten abstract, a front-matter "Contributions, Scope & Limitations" section, and the threshold-sweep verdict in §7. A later small standing-water comparison withdrew the interim positive LBI claim: four brine sites overlapped three freshwater controls, so LBI is now the Liquid/Salinity Response Index and remains a screening hypothesis. The paper leads with the honest negative result rather than "five novel detectors." Remaining author choices are voice/venue and whether to pursue a different sensing modality as follow-on research.
