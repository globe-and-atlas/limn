...
## Deep Fusion Index Shows Nothing

APEX and HPWI require a time range. Check in DevTools that the `time` param is `YYYY-MM-DD/YYYY-MM-DD` (not a single date). If single-date, the 30-day expansion in `getWMSLayer()` didn't trigger — verify `activeIdx === 'apex'` is in the condition.
...
1. Check console for the specific error line in `generateReport()` in report.js.
