...
The shipped viewer defaults to True Color and treats all index outputs as evidence lenses. Analysts first verify the documented site/date, clear-pixel quality, and visible surface; then use before/after imagery and established contextual formulas (MNDWI, AWEIsh, NDMI, SAVI, BSI, NDRE, dual-SWIR contrast, and SWIR false color). LBI is a preliminary liquid/salinity-response hypothesis. PWCI, ASAI, and OBEC are retained in a collapsed negative-result study drawer for reproducibility, not promoted as primary detectors. Agreement among correlated optical responses is not independent confirmation.
