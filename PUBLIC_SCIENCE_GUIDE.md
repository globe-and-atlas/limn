...
The honest conclusion is that Limn's flagship composites, in both configurations shipped to date, **do not yet demonstrate simultaneous useful recall and low false-positive rate**. The multi-gate architecture is a sound design principle (Section 3), but the calibration that realizes it as a genuine discriminator has not yet been found. Accordingly this release makes **no detection-accuracy or false-positive claim**; Limn is presented as an experimental screening methodology and an investigative-lead tool, not a validated detector. Full per-index figures and method: `reports/preprint_qc_2026-07-19.md`, `execution/false_positive_summary.md` (pipeline), and `execution/viewer_false_positive_summary.md` (viewer).
...
Honesty about where the work stands, however, is essential. As Section 7 documents, the two calibrations implemented so far do not yet realize that principle as a usable detector: one reaches high recall only by firing on nearly all background, the other suppresses background only by firing on almost nothing (including real spills). Finding a calibration — of thresholds, gains, and gate structure — that delivers useful recall *and* a low false-positive rate simultaneously is the central open problem, and it is unsolved as of this revision. Accordingly Limn is presented as an **experimental screening methodology and investigative-lead tool**, not a validated detector: any positive is a lead to be confirmed with field or higher-resolution data, never a standalone determination, and the quantitative performance figures here are calibration diagnostics rather than accuracy claims.
