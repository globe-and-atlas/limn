...
function evaluatePixel(sample) {
  ...
  return [vv, vh, (vv/(vh+0.001))*0.5, 1];
    ...
    // Note the LAYERS parameter now points to the user's new Layer ID
    ...
    if (text.includes("<ServiceException>")) {
        ...
        console.log("SUCCESS! Got Image Data (first 100 bytes):", Buffer.from(text).toString('hex').substring(0, 100));
    }
}
test();
