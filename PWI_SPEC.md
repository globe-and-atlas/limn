...
The Permian Basin's high albedo (brightness) from white/tan caliche sand naturally spikes SWIR reflectance, which conventionally triggers false positives in algorithmic mapping. The PWI mathematically subtracts this "desert noise floor."
...
* **Brine Score:** `Math.max(0, NDSI - 0.10)` (Dry Permian soil is 0.05–0.10; salt/brine pushes past 0.10)
* **Hydrocarbon Score:** `Math.max(0, (HCAI - 0.30) * 2)` (Red dirt peaks at 0.25–0.30; oil contamination starts at 0.40+. Cubic scaling eliminates borderline bare-soil leakage)
* **Heavy Metal Score:** `Math.max(0, (HMRI - 2.0) * 2)` (Caliche sits at ~1.0, normal soil 1.5–1.9; contamination pushes past 2.0)
